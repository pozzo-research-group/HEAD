...
class TestShapeMatchBO:
    def __init__(self):
        ...
        if SPECTRA=='saxs':
            ...
            self.xt, self.yt = self.sim.get_uvvis(shape_param = SHAPE_PARAM, n_samples=N_UVVIS_SAMPLES)
        ...
        self.random_x   = self.draw_random_batch(n_samples=N_INIT_SAMPLES)
    ...
    def oracle(self, x):
        ...
        logging.info('Evaluation %d : %.2f'%(self.EXPT_ID, dist))
        self.expt[self.EXPT_ID] = [xi, yi, dist]
        self.EXPT_ID += 1

        return torch.tensor([dist+1e-6])
    ...
    def get_model(self,train_x, train_obj):
    ...
    def run(self, metric):
        ...
        self.EXPT_ID = 0
        self.expt = {}
        self.train_x = self.random_x
        self.train_obj = self.batch_oracle(self.train_x)
        
        for iteration in range(1, N_ITERATIONS + 1):  
            mll, model = self.get_model(self.train_x, self.train_obj)  
            ...
            logging.info('Iteration : %d/%d mean objective : %.2f'%(iteration, 
                N_ITERATIONS, -new_obj.mean().cpu().numpy()))
